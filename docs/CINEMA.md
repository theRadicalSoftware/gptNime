...
- A dedicated Chrome or Brave session follows WCO’s normal preparation flow in the background, remembers its own session between episodes and minimizes when the video is ready. Preparation never calls the focus action; **Show WCO window** is an explicit user action. Complete any human verification there. The cinema shows **Waiting for WCO verification** and **Show WCO window** while waiting; verification has a separate allowance of up to ten minutes instead of consuming the two-minute media deadline. If the browser requires another gesture for playback, the cinema displays a **Press play** message. Stable series/episode/neighbor pages are remembered; signed media sources are freshly resolved each time.
...
npm run test:wco
npm run test:wco-session # desktop display required
```
