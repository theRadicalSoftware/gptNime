...
try {
  ...
  const prediction = task(), opened = task()
  const predictionResult = queue.prefetch('brave:prediction:sub', prediction.work).catch(() => 'cancelled')
  await tick()
  const openedResult = queue.prefetch('brave:opened:dub', opened.work, true)
  await tick()
  assert.equal(await predictionResult, 'cancelled')
  assert.equal(opened.starts, 1)
  const adopted = queue.resolve('brave:opened:dub', opened.work, new AbortController().signal, 'opened')
  assert.throws(() => queue.prefetch('brave:hover:sub', task().work, true), PreparationBusy)
  assert.equal(opened.aborted, false)
  opened.finish('opened')
  assert.equal(await adopted, 'opened'); assert.equal(await openedResult, 'opened')
  assert.equal(opened.starts, 1)
  console.log('✓ Opening a title replaces an unrelated background prediction; Play adopts it and cannot be interrupted by another hover')
